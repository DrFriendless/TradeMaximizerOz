...
public class WantList {
    ...
    private Map<String, Integer> spendingLimits = new HashMap<String, Integer>();
    ...
    void showUnsubmitted() {
        ...
        Set<String> submittedUsers = new HashSet<String>();
        for (TradeRequest want : wants) {
            ...
            if (item == null) {
                errors.add(want.getUserName() + ": item " + itemName + " you are trying to trade away does not exist.");
                continue;
            }
            allCodes.remove(item.getTradeCode());
            allUsers.remove(item.getUserName());
            submittedUsers.add(item.getUserName());
        }
    ...
    public String satisfiesSpendingLimits(Map<String, Double> paid) {
        ...
        for (Map.Entry<String, Double>  entry : paid.entrySet()) {
            Integer limit = spendingLimits.get(entry.getKey());
