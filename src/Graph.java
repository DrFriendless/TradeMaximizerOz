...
public class Graph {
    ...
    public Vertex addVertex(String name,String user,boolean isDummy,boolean isMoney, TradeRequest request) {
        assert !frozen;
        assert getVertex(name) == null;
        Vertex receiver = new Vertex(name,user,isDummy,isMoney,VertexType.RECEIVER, request);
        receivers.add(receiver);
        nameMap.put(name,receiver);

        Vertex sender = new Vertex(name+" sender",user,isDummy,isMoney,VertexType.SENDER, request);
        senders.add(sender);
    ...
    public Edge addEdge(Vertex receiver,Vertex sender,long cost, TradeRequest request) {
        assert !frozen;
        Edge edge = new Edge(receiver,sender,cost, request);
    ...
    List<Cycle> findCycles() {
        ...
        for (Vertex vertex : RECEIVERS) {
            ...
            while (v.mark != timestamp) {
                v.mark = timestamp;
                cycle.add(v);
                cost += getCost(v.match, v);
                v = v.match.twin;
            }
            cost += getCost(v.match, v);
            cycles.add(new Cycle(cycle, cost));
        }
        return cycles;
    } // end findCycles
    
    private long getCost(Vertex v1, Vertex v2) {
        Edge e = getEdge(v1, v2);
        if (e == null) e = getEdge(v2, v1);
        if (e == null) return 0;
        return e.cost;
    }

    <T> void shuffle(T[] a, Random random) {
    ...
    void shuffle(Random random) {
        ...
        for (Vertex v : RECEIVERS) shuffle(v.EDGES, random);
